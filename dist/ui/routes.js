...
uiRouter.get('/dashboard', requireAuth, async (req, res, next) => {
    ...
    try {
        ...
        const topChannelAgg = await prisma.event.groupBy({ by: ['channel'], _count: true, where: { ...where, recovered: true }, orderBy: { _count: { channel: 'desc' } }, take: 1 }).catch(() => []);
