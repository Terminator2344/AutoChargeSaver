...
export async function notify(options) {
    ...
    if (to.telegramId && env.ENABLE_TELEGRAM) {
        if (!env.TELEGRAM_BOT_TOKEN) {
            ...
            promises.push(sendTelegram(to.telegramId, message)
                .then((result) => {
                if (result.messageId) {
                ...
                .catch((error) => {
                logger.error({ telegramId: to.telegramId, error: error?.message || String(error) }, '[Telegram ERROR]');
    ...
    if (to.email && env.ENABLE_EMAIL) {
        if (!env.SMTP_HOST || !env.SMTP_PORT) {
            ...
            promises.push(sendEmail(to.email, emailSubject, message, html)
                .then((result) => {
                if (result.messageId) {
                ...
                .catch((error) => {
                logger.error({ email: to.email, error: error?.message || String(error) }, '[Email ERROR]');
                // Не пробрасываем ошибку, чтобы не прервать отправку через другие каналы
            }));
        }
    }
